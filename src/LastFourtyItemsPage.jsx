...
import {LastFourtyItemsData} from "./Api"


function LastFourtyItemsPage() {
  const[loading,setLoading]=useState(true);
  const[productlist,setProductlist]=useState([]);
  
  useEffect(function(){
  const promise = LastFourtyItemsData();
...
function handleQuerry(event){ setQuerry(event.target.value);
  }
...
setSort(event.target.value);
 ...
 if (loading) {
  ...
  return (
   ...
   <div className="p-2 bg-white max-w-6xl mx-auto shadow-2xl py-[50px] my-[60px] px-9 ">
...
export default LastFourtyItemsPage;